...
(function() {
  ...
  this.base_url = function(url) {
    ...
    if (is_production()) {
      return Config.remote_web_service + url;
    }
    return Config.local_web_service + url;
  };

  this.change_back_functionality = function(func, selector) {
    if (selector == null) {
      selector = '.back';
    }
    return $(selector).removeAttr('data-navigation').click(func);
  };

  this.restore_back_functionality = function(func, selector) {
    if (selector == null) {
      selector = '.back';
    }
    return $(selector).attr('data-navigation', '__back__');
  };
  ...
  this.ensure = function(item, selector) {
    if (item.is(selector)) {
      return item;
    }
    return item.parents(selector);
  };
  ...
  this.meaningfy_url = function(url) {
    if (!/http/.test(url)) {
      return site_url(url);
    }
    return url;
