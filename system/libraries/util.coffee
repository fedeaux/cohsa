...
@base_url = (url = '') ->
  return Config.remote_web_service + url if is_production()
  Config.local_web_service + url

@change_back_functionality = (func, selector = '.back') ->
  $(selector)
    .removeAttr('data-navigation')
    .click func

@restore_back_functionality = (func, selector = '.back') ->
  $(selector).attr 'data-navigation', '__back__'

@fnn = (item...) ->
  item[1]

@ensure = (item, selector) ->
  if item.is selector
    return item
...
@meaningfy_url = (url) ->
  return site_url url unless /http/.test url
  url
